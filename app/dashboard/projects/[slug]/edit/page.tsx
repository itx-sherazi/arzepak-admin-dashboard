  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setPaymentPlans(Array.isArray(p.paymentPlans) ? (p.paymentPlans as { label: string; images: string[] }[]) : []);
        setBookingForm(Array.isArray(p.bookingForm) ? (p.bookingForm as { label: string; images: string[] }[]) : []);
        setCatalogue(Array.isArray(p.catalogue) ? (p.catalogue as { label: string; images: string[] }[]) : []);
              ...
              <div className="space-y-4">
                {paymentPlans.map((pp, i) => (
                  <div key={i} className="border border-gray-200 rounded-xl p-4 bg-gray-50/50">
                    ...
                    {pp.images.length > 0 && (
                      <p className="text-[11px] text-gray-400 mt-1.5">{pp.images.length} page{pp.images.length > 1 ? "s" : ""} uploaded</p>
                    )}
                  </div>
                ))}
              </div>
